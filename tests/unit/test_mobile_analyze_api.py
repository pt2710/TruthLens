...
def _mock_signals(*, music_likelihood: float) -> ModelSignals:
    return ModelSignals(
        text_score=0.52,
        vision_score=0.61,
        metadata_score=0.48,
        ...
        mode="trained",
        feature_summary={"music_likelihood": music_likelihood},
    )
...
def test_mobile_analyze_share_returns_structured_payload_for_video_with_transcript(
    ...
    monkeypatch.setattr(
        ...
        lambda payload: _mock_score_result(action="ask-report", risk_score=0.74, confidence=0.88, uncertainty=0.12),
    )
...
def test_mobile_analyze_share_uses_heuristic_drafts_when_gemini_is_unavailable(
    ...
    monkeypatch.setattr(
        "truthlens_api.main.score_item",
        lambda payload: _mock_score_result(action="badge", risk_score=0.31, confidence=0.7, uncertainty=0.2),
    )
...
def test_mobile_analyze_share_recommends_verify_for_music_context(
    ...
    monkeypatch.setattr("truthlens_api.main.predict_item_signals", lambda payload: _mock_signals(music_likelihood=0.93))
    monkeypatch.setattr(
        "truthlens_api.main.score_item",
        lambda payload: _mock_score_result(action="none", risk_score=0.14, confidence=0.89, uncertainty=0.11),
    )
    monkeypatch.setattr("truthlens_api.main.gemini_available", lambda: False)
    monkeypatch.setattr(
        "truthlens_api.main._build_heuristic_suggestion_response",
